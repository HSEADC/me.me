...
function syncOutlinedText() {
  document.querySelectorAll(".txt, .hd, .nv, .A_HeaderPart, .A_Question, .A_NumberOfQuestion, .A_TestGuessResultHeader, .A_TestGuessResultText, .A_AnswerText").forEach(function (el) {
...
function selectAnswer(index, dom) {
  ...
  dom.checkBoxes.forEach(function (checkBox, checkBoxIndex) {
    checkBox.checked = checkBoxIndex === index;
  });
}
function bindAnswerSelection(dom) {
  dom.answerRows.forEach(function (row, index) {
    row.addEventListener("click", function (event) {
      var clickedCheckbox = event.target instanceof HTMLInputElement;
      if (!clickedCheckbox) {
        selectAnswer(index, dom);
      }
    });
  });
  dom.checkBoxes.forEach(function (checkBox, index) {
...
function renderStage() {
  ...
  dom.numberOfQuestion.innerText = "".concat(currentStage + 1, "/").concat(stages.length);
  dom.question.innerText = stage.question;
  dom.imageBlock.style.backgroundImage = "url(\"".concat(stage.blurredImage, "\")");
  dom.answerTexts.forEach(function (answerText, index) {
    answerText.innerText = stage.answers[index] || "";
  });
  dom.checkBoxes.forEach(function (checkBox) {
...
function buildResultLayout(isCorrect) {
  ...
  var resultImage = document.querySelector(".A_TestGuessImage");
  var resultHeader = document.querySelector(".A_TestGuessResultHeader");
  var resultText = document.querySelector(".A_TestGuessResultText");
  var nextButton = document.querySelector(".A_TestGuessNextButton");
  resultImage.style.backgroundImage = "url(\"".concat(stage.normalImage, "\")");
  if (isCorrect) {
...
function bindSubmit(dom) {
  ...
  dom.submitButton.onclick = function () {
    if (selectedAnswerIndex === null) return;
    var isCorrect = selectedAnswerIndex === stages[currentStage].correctAnswerIndex;
    buildResultLayout(isCorrect);
  };
}
function restoreQuestionLayout() {
  var questionContainer = document.querySelector(".W_TestGuessQuestion");
