...
document.addEventListener("DOMContentLoaded", function () {
  var indexCards = document.querySelectorAll(".O_ArticlesCards a");
  if (!indexCards.length) return;
  indexCards.forEach(function (link) {
    var href = link.getAttribute("href") || "";
    var articleIdMatch = href.match(/art_\d+/);
    var articleId = articleIdMatch ? articleIdMatch[0] : "";
    var article = articles_namespaceObject.find(function (item) {
      return item.id === articleId;
    });
    if (!article || !article.description) return;
    var card = link.querySelector(".A_ArticleCard");
    if (!card) return;
